...
function mkPubkeyHashReplayScript(address, pubKeyHash, assetSatoshi, assetName) {
  ...
  if (assetSatoshi > 0) {
    ...
    return zopcodes.OP_DUP + zopcodes.OP_HASH160 + zbufferutils.getPushDataLength(subAddrHex) + subAddrHex + zopcodes.OP_EQUALVERIFY + zopcodes.OP_CHECKSIG + zopcodes.OP_RVN_ASSET
      + zbufferutils.getPushDataLength(zopcodes.RVN_R + zopcodes.RVN_V + zopcodes.RVN_N + zopcodes.RVN_T + zbufferutils.getPushDataLength(assetNameHex) + assetNameHex + assetSatoshiHexReversed + nullString)
      + zopcodes.RVN_R + zopcodes.RVN_V + zopcodes.RVN_N + zopcodes.RVN_T + zbufferutils.getPushDataLength(assetNameHex) + assetNameHex + assetSatoshiHexReversed + nullString
      + zopcodes.OP_DROP;
  }
  // Minimal encoding
